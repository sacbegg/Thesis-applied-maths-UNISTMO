...
%$$
%\begin{array}{|l|l|}
%	\hline \multicolumn{1}{|c|}{\text { Names }} & \multicolumn{1}{|c|}{\text { Formula }} \\
%	\hline \text { Distancia euclidiana } & \|a-b\|_{2}=\sqrt{\sum_{i}\left(a_{i}-b_{i}\right)^{2}} \\
%	\hline \text { Distancia euclidiana al cuadrado } & \|a-b\|_{2}^{2}=\sum_{i}\left(a_{i}-b_{i}\right)^{2} \\
%	\hline \text { Distancia Manhattan } & \|a-b\|_{1}=\sum_{i}\left|a_{i}-b_{i}\right| \\
%	\hline \text { distancia máxima } & \|a-b\|_{\infty}=\max _{i}\left|a_{i}-b_{i}\right| \\
%	\hline \text { Distancia de Mahalanobis } & \sqrt{(a-b)^{\top} S^{-1}(a-b)} \text { donde } S \text { es la matriz de covarianza } \\
%	\hline \text { Similitud coseno } & \frac{a \cdot b}{\|a\| b \|} \\
%	\hline
%\end{array}
%$$

El criterio de enlace determina la distancia entre conjuntos de observaciones como una función de las distancias entre observaciones dos a dos. Algunos criterios de enlace entre dos conjuntos de observaciones A y B frecuentemente usados son las mostradas en \autoref{crit_enlance}
